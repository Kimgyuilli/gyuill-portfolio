...
재고가 100개인 상품에 동시 주문이 몰리면, 재고 확인과 차감 사이의 틈에서 여러 트랜잭션이 같은 재고를 읽고 각자 차감해 **오버셀링**이 발생한다. 단일 트랜잭션의 격리 수준만으로는 막기 어렵고, 막더라도 처리량이 급격히 떨어진다.

### 해결 (To-Be)

**Redis 분산 락(Redisson)으로 동시 요청을 직렬화**하고, Redis 장애·만료에 대비해 **DB 낙관적 락(\`@Version\`)을 최후 방어선**으로 두는 이중 방어를 적용했다.

\`\`\`mermaid
flowchart TB
    R["주문 요청"] --> L{"Redis 분산 락 획득?"}
    L -->|"획득 실패"| E["즉시 409 응답 (대기 없이 실패)"]
    L -->|"획득 성공"| T["트랜잭션 시작"]
    T --> D["재고 차감 — @Version 낙관적 락"]
    D --> V{"version 충돌?"}
    V -->|"충돌"| RB["롤백 → 재시도 / 실패 처리"]
    V -->|"정상"| C["커밋"]
    C --> U["트랜잭션 커밋 후 락 해제"]
\`\`\`
...
분산 락이 정상 동작하는 한 충돌은 나지 않는다. 하지만 Redis 장애·TTL 만료·네트워크 분할 같은 상황에서 락이 무력화될 수 있다. 이때 \`@Version\` 낙관적 락이 **DB 레벨의 최후 방어선**으로 작동해 오버셀링을 막는다. 두 락은 중복이 아니라 서로 다른 실패 모드를 막는다.
...
| 장애 대응 | DB 단일 의존 | Redis 실패 시 낙관적 락 fallback |
...
flowchart TB
    ...
    OB -. "한계 초과 시 FAILED" .-> SL["Slack 알림"]
    K -->|"at-least-once"| C["Consumer — processed_events UK로 중복 차단"]
    C -->|"3회 재시도 실패 (1s · 5s · 30s)"| DLQ[("DLQ")]
    DLQ --> SL
\`\`\`
...
1,000 VUser 동시 주문에서 인프라 한계로 처리량 임계값(요청 실패율)은 미달했지만, **오버셀링 0건 · 재고 정합성 OK**는 달성했다. "처리량은 인프라를 키우면 되지만 정합성은 설계가 틀리면 깨진다"는 관점에서, 정합성 PASS를 핵심 성과로 분리해 기록했다. 동시에 HPA가 Pod를 1→3으로 늘려 CPU를 90%→15%로 안정화하는 것까지 확인했다.
...
이 프로젝트는 코드만 설계한 게 아니라, **AI와 함께 일하는 흐름 자체를 설계**했다. Claude로 계획하고 Codex로 리뷰를 따로 쓰다 보니 도구를 오갈 때마다 문맥을 다시 설명해야 했고, "계획서엔 있는데 구현엔 빠진" 괴리가 반복됐다. 모델 성능보다 **상태 관리와 프로세스 부재**가 진짜 문제라고 보고, 작업 흐름을 묶는 하네스를 직접 만들었다.

핵심은 작업을 \`/plan → /work → /ship\` 3단계로 고정하고, task마다 \`.state.json\`으로 상태를 잇는 것이다. **Claude는 계획·구현·오케스트레이션**을, **Codex는 독립 리뷰어**(diff·계획서를 output-schema로 강제된 JSON으로 응답)를 맡는다. 사람의 개입은 흩어지지 않고 **정해진 게이트(GW·GS)**에서만 일어난다.

\`\`\`mermaid
flowchart TB
    CL["Claude — 계획·구현·오케스트레이션"]
    subgraph F["/plan → /work → /ship (.state.json 으로 연결)"]
      direction LR
      P["/plan<br/>계획"] --> W["/work<br/>구현"] --> S["/ship<br/>커밋·PR"]
    end
    CL --> F
    P -. "계획 리뷰" .-> CX["Codex<br/>독립 리뷰"]
    W -. "diff 리뷰" .-> CX
    CX -. "findings" .-> G["사용자 게이트<br/>GW·GS"]
    G -. "결정 반영" .-> CL
\`\`\`

| 장치 | 내용 |
| --- | --- |
| 상태 추적 | task별 \`.state.json\` (stage · 세션 · 리뷰 이력 · diff 경로 · 커밋 계획). 중단돼도 **재진입 매트릭스**로 복구 |
| 독립 리뷰 | Codex가 diff/계획서를 스키마 강제 JSON으로 리뷰 → **P0(머지 차단)/P1/P2** 분류 |
| 사용자 게이트 | GW(work)·GS(ship) 지점에서만 사람이 결정. P0를 무시하려면 **사유 입력 필수** |
| 대용량 diff | 500줄↑ split 리뷰(≤3 chunk), 실패 시 degraded 처리 + 위험도 분류(auth/payment touch 등) |
| 정합성·안전 | atomic state 치환, lock, \`git add -A\` 금지(파일 단위 커밋), **drift detector**(state↔git 괴리 감지) |
| 비용 제어 | Codex 호출 상한(work ≤3, cycle ≤5), 메트릭 TSV(토큰·wall time) 기록 |

정량 KPI를 앞세우기보다 "체감상 편한가 · 불필요한 질문이 줄었나 · 다시 쓰고 싶은가"를 1차 기준으로 삼은 **v0**다. 계획 → 구현 → 커밋 partition → PR → archive까지 이어지는 전체 흐름을 smoke 테스트로 검증했다. shared-logic 셸 함수(~1,300줄)와 bats 테스트로 하네스 자체의 동작도 보장한다.
...
export const peekcart: Project = {
  title: 'PeekCart',
  description:
    '대용량 트래픽을 고려해 모놀리식에서 MSA로 단계적으로 진화시키는 이커머스 백엔드 플랫폼',
  projectType: 'Main',
