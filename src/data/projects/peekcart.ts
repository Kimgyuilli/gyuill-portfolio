...
1,000 VUser 동시 주문에서 인프라 한계로 처리량 임계값(실패율 Run 1 60.59% · Run 2 35.90%, 합격선 <10%)은 미달했지만, **오버셀링 0건 · 재고 정합성 OK**는 달성했다. 이번 실험에서는 처리량과 정합성을 분리해 해석했다. 처리량 미달은 노드·Pod 증설 여지가 있는 반면, 오버셀링 0건은 이중 방어 중 최후 방어선이 실제로 작동한 결과다. HPA는 CPU 400% saturation에서 Pod를 1→3으로 늘렸고, scale-out 직후 90%, 부하 종료 후 15%로 내려갔다.
