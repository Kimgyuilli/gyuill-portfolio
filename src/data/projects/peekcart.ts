...
- **1,000 VUser 동시 주문에서 재고 정합성 OK · 오버셀링 0건.** Redis 분산 락과 DB 낙관적 락으로 검증했다 (처리량 합격선은 미달 — 3장에 그대로 기록)
...
| 검증 | 미검증 | 1,000 VUser 동시 주문 **오버셀링 0건** (처리량은 합격선 미달) |
...
1,000 VUser 동시 주문에서 인프라 한계로 처리량 임계값(실패율 Run 1 60.59% · Run 2 35.90%, 합격선 <10%)은 미달했지만, **오버셀링 0건 · 재고 정합성 OK**는 달성했다. 이번 실험에서는 처리량과 정합성을 분리해 해석했다. 처리량 미달은 노드·Pod 증설 여지가 있는 반면, 오버셀링 0건은 설계가 보장한 결과다. HPA는 CPU 400% saturation에서 Pod를 1→3으로 늘렸고, scale-out 직후 90%, 부하 종료 후 15%로 내려갔다.

다만 **이 실험의 검증 범위는 분명히 해둔다.** 경합 상품 10개 × 재고 100(총 1,000개)에 대해 커밋까지 도달한 주문은 Run 1 25건 · Run 2 110건이라, 재고가 소진되는 지점까지 경합이 몰리지는 않았다. 오버셀링 0건은 "락이 재고 고갈 경계를 지켜냈다"는 증명이 아니라 "정합성이 깨질 조건에서도 깨지지 않았다"는 1차 확인에 가깝다. 병목은 MySQL 커넥션 풀과 분산 락 contention으로 식별했고, 인프라를 보강해 재고 고갈 경계까지 밀어붙이는 재측정을 후속 과제로 남겼다.
...
> 관련 글: 학습 기록 13 (CI · Docker), 학습 기록 14 (Kustomize · GKE), 학습 기록 15 (관측성)
