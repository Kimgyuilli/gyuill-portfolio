...
export const resumeProjects: ResumeProject[] = [
    ...
    cases: [
      {
        ...
        result:
          '50 VUser 5분 기준 TPS 265 → 613(×2.31), 평균 응답 188ms → 82ms. 목표 3배에 미달한 원인을 캐시 ON에서도 Pod CPU가 request 대비 175%라는 데서 특정해 다음 검증 대상을 CPU로 결정했습니다. 부하를 키우자 CPU가 limit(request 대비 400% = 2 vCPU)에 걸려 HPA가 replica를 1→3으로 늘렸지만, 신규 Pod가 Ready에 도달하기까지 65초가 걸렸고 그동안 기존 Pod마저 readiness를 잃어 63초간 엔드포인트에서 빠졌습니다. replica 3을 미리 띄워 같은 부하를 재측정하니 커밋된 주문이 25 → 110건(×4.4)으로 늘어, Scale-out과 반응 공백은 별개라는 것을 대조로 확인했습니다.',
        links: [
          {
      ...
      {
        title: '예상과 달랐던 분산락의 1,000 VUser 동시 주문의 오버셀링 0건 측정',
        problem:
          '재고 확인과 차감 사이의 틈에서 여러 트랜잭션이 같은 재고를 읽으면 오버셀링이 발생합다. SELECT FOR UPDATE로 막을 수는 있지만 락 경합과 커넥션 점유를 주문 경로에 집중시키고 싶지 않았습니다.',
        judgment:
          'Redis 분산 락으로 직렬화하고 경합 시 대기 없이 409를 반환하되 Redis 장애 시에는 락 없이 진행하고 DB 낙관적 락(@Version)이 최후 방어선이 되게 설계 했습니다. 두 락은 중복이 아니라 서로 다른 실패를 막습니다. 그리고 "통과했다"로 끝내지 않고 무엇이 실제로 막았는지를 SQL로 추적했습니다.',
        result:
      ...
      },
      {
        ...
        result:
          '"선점만 되고 처리는 실패해 영원히 건너뛰는" 구멍이 없는 구조가 됐습니다. 발행 실패(Outbox FAILED + Slack 알림)와 소비 실패(3회 재시도 후 DLQ 격리)는 원인도 대응도 다르므로, 같은 메커니즘으로 보지않고 분리했습니다.',
        links: [
          {
  ...
  },
  {
    ...
    note: 'SOPT 38기 앱잼 프로덕트입니다. HTTP 매핑 76개짜리 Go 서버를 멈추지 않고 옮기는 중이며, 팀 간 합의와 설계 결정을 문서로 기록하고 있습니다.',
    cases: [
      {
        title: '이관의 제약을 인지하고 계획을 유연하게 수정',
        problem:
        ...
        judgment:
          '초기엔 빅뱅 컷오버를 채택했습니다. 하지만 완성 전까지 운영 가치가 0이고 리스크가 전환 시점 하나에 몰린다는 문제가 있었습니다. 이에 요청 왕복을 반복하는 대신 스키마 소유권 자체를 신규 서버로 가져오는 방향으로 전환했습니다. 그리고 옮긴 것이 같은지를 수동으로 판정하는 게 아닌, 두 서버를 같은 픽스처로 나란히 띄워 응답과 DB 기록을 대조하는 하네스를 만들고 그 위에 OpenAPI 스냅샷과 배포 후 스모크를 구축해 검증을 3층으로 나눴습니다.',
        result:
          '운영과 같은 형상의 DB로 리허설한 뒤 소유권을 이전해 마이그레이션 이력 42건을 실패 0으로 반영했습니다. 트래픽 전환은 아직 진행중입니다. 병행 예정이던 프런트·워커 작업이 진행되지 못해 경로별로 조금씩 돌릴 라우팅 지점이 없었고 그래서 점진 전환을 기각하고 인증과 Product API 2단계로 나눈 뒤 무엇이 실제로 전환을 막는지를 게이트 셋으로 재정의했습니다. 현재는 그 게이트를 여는 프런트/레거시 작업까지 범위를 넓혀 순차적으로 진행 중입니다.',
        links: [
          {
