...
export const resumeIntro: string[] = [
  ...
  '개선됐다는 감각에 의존하지 않고 측정합니다. 캐시로 상품 조회 TPS를 2.31배 올리고, 목표 3배에 미달한 다음 병목을 CPU로 좁혔습니다.',
...
export const resumeProjects: ResumeProject[] = [
    ...
    cases: [
      {
        ...
        result:
          '50 VUser 5분 기준 TPS 265 → 613(×2.31), 평균 응답 188ms → 82ms. 목표 3배에 미달한 원인을 캐시 ON에서도 Pod CPU 피크가 1.75 vCPU(limit 2 vCPU의 88%)라는 데서 특정해 다음 검증 대상을 CPU로 결정했습니다. \n부하를 키우자 CPU가 limit 2 vCPU에 걸려 HPA가 replica를 1→3으로 늘렸지만 신규 Pod가 Ready에 도달하기까지 65초가 걸렸습니다. replica 3을 미리 띄워 같은 부하를 재측정하니 커밋된 주문이 25 → 110건(×4.4)으로 늘어, scale-out과 반응 공백은 별개라는 것을 대조로 확인했습니다.',
        links: [
          {
      ...
      },
      {
        ...
        result:
          '재고 100개 상품 10개에 1,000 VUser를 몰아 오버셀링 0건/정합성 OK라는 결과를 얻었지만 실제 방어선은 낙관적 락이었습니다. 주문 서비스의 클래스 단위 @Transactional에 재고 차감이 합류해 락 해제가 커밋보다 먼저 일어나 직렬화 효과가 사라져 있었습니다. 단위 테스트에서는 잡히지 않았고 부하 테스트에서 찾을 수 있었습니다. \n처리량은 실패율 35.9%로 목표(<10%)에 미달해, 병목을 DB 커넥션 풀과 Pod readiness 두 갈래로 좁혀 후속 과제로 기록했습니다.',
        links: [
          {
      ...
      },
      {
        ...
        judgment:
          '비즈니스 데이터와 이벤트를 하나의 트랜잭션으로 Outbox에 저장했습니다. 발행 방식은 Debezium CDC가 지연이 짧지만 Kafka Connect 클러스터를 추가로 운영해야 해서 폴링을 택하되 추후 교체 가능한 구조로 분리했습니다. \n멱등성은 processed_events 복합 UK를 처리 전에 선점 INSERT 하는 방식으로 두되 그 INSERT를 비즈니스 로직과 같은 트랜잭션에 넣어 처리 실패 시 함께 롤백되게 했습니다.',
        result:
          '"선점만 되고 처리는 실패해 영원히 건너뛰는" 구멍이 없는 구조가 됐습니다. 발행 실패(Outbox FAILED + Slack 알림)와 소비 실패(3회 재시도 후 DLQ 격리)는 원인도 대응도 달라 같은 메커니즘으로 보지 않고 분리했습니다.',
        links: [
          {
  ...
  },
  {
    ...
    note: 'SOPT 38기 앱잼 프로덕트입니다. HTTP 매핑 96개짜리 Go 서버를 이관 중이며, 팀 간 합의와 설계 결정을 문서로 기록하고 있습니다.',
    cases: [
      {
        title: '스키마 소유권 이전과 전환 단위 재설계',
        problem:
          '레거시는 프로덕션만 2만 줄, HTTP 매핑 96개였고 신규 서버와 같은 PostgreSQL을 공유했습니다. \n스키마 소유권이 레거시에 있어 운영에서는 마이그레이션을 돌리지 못하고 검증만 했고 순서가 바뀌면 서버가 아예 기동하지 않았습니다. 레거시에 반영을 요청하는 왕복이 이관의 병목이었습니다.',
        judgment:
          '초기에 빅뱅 컷오버를 고려하다가 기각했습니다. 완성 전까지 운영 가치가 없고 리스크가 전환 시점 하나에 몰리기 때문에 레거시에서 기능을 하나씩 새 서버로 넘기는 Strangler Fig 방식을 택했습니다. 이때 구현은 한 사용자 행위를 끝까지 옮기는 최소 수직 슬라이스를 단위로 삼고, 트래픽 전환은 REST 핸들러를 다 옮겨도 같은 테이블을 쓰는 MCP, 봇 경로가 레거시에 남아 있으면 안 되기 때문에 aggregate 하나를 단위로 정했습니다. \n그리고 레거시에 반영을 요청하는 왕복을 되풀이하는 대신 스키마 소유권 자체를 신규 서버로 가져오는 쪽으로 방향을 바꿨습니다.',
        result:
          '운영과 같은 형상의 DB로 리허설한 뒤 소유권을 이전해 마이그레이션 이력 42건을 실패 0으로 반영했습니다. \n웹이 실제로 쓰는 Product API 33개는 전부 구현했지만 병행 예정이던 FE·워커 작업이 진행되지 못해 경로별로 조금씩 돌릴 라우팅 지점이 없었습니다. 그래서 capability별 점진 전환을 기각하고 인증과 Product API 2단계로 나눴습니다.',
        links: [
          {
      ...
      },
      {
        ...
        judgment:
          '레거시에 테스트가 없어 "무엇이 맞는가"의 기대값이 될 수 있는 건 레거시의 실제 동작뿐이었습니다. 그래서 신규 서버만 검증하는 방식 대신 두 서버를 같은 픽스처로 띄워 같은 요청을 보내고 결과를 대조하는 하네스를 만들었습니다. \n구축 과정에서 값이 바뀌지 않는 PATCH에서 레거시는 조건 없이 updated_at을 갱신하고 신규 서버는 JPA dirty checking으로 UPDATE가 발생하지 않는 차이를 발견하게 됐습니다. 이러한 경우는 응답 비교로 차이를 잡을 수 없어 비교 대상에 요청 이후 DB 기록까지 포함했습니다. \n하지만 이 대조가 보장하는 건 "두 서버가 같다"까지이고, 둘 다 틀렸거나 레거시에 대응물이 없는 신규 API는 비교 대상 자체가 없습니다. 그래서 계약은 OpenAPI 스냅샷을 커밋해 PR diff와 CI로 잡고, 실제 환경 동작은 배포 후 스모크로 나눴습니다.',
        result:
          '판정 기준을 golden 파일로 고정해 케이스마다 통과와 실패가 자동으로 구분되게 했습니다. 이후 고쳤던 버그를 일부러 되돌려 해당 케이스가 불일치로 판정되고 종료 코드가 1이 되는 것까지 확인했습니다. \n만드는 과정에서는 도구가 잡을 수 없는 경로(양쪽 서버가 모두 죽으면 상태 코드가 일치해 전 케이스 통과, 빌드 캐시가 스냅샷 게이트를 복원)를 찾아 고쳤습니다.',
      },
      /* 분량 조절로 잠시 내려둔 케이스 (2026-09-10).
         목표 4장에 맞추려고 뺐다. 이 블록의 주석만 풀면 그대로 5장으로 돌아온다.
         뺀 이유: 이 케이스의 메시지(규칙을 도구가 지키게 만든다)가 소개 셋째 줄,
         PeekCart note, Momens 2번 케이스에 이미 세 번 나온다. 나머지 케이스는 대체재가 없다.
         Spring Modulith 노출이 필요한 공고라면 되살릴 것. */
      // {
      // title: '모듈 경계의 빌드 강제',
      // problem:
      // '패키지를 나눠도 다른 모듈의 내부 클래스를 import 하는 순간 경계가 깨지게 됩니다. 이를 막는 건 사람의 리뷰뿐이라 사람이 늘고 일정이 급해질수록 가장 먼저 지켜지지 않을 규칙이었습니다.',
      // judgment:
      // '경계를 빌드에서 검사하게 만들었습니다. 1차로 각 모듈을 Gradle 서브프로젝트로 분리해 컴파일 클래스패스에 없으면 참조 자체를 불가능하게 하고, 2차로 Spring Modulith 검증으로 클래스패스 안에서 일어나는 internal 참조와 순환 의존을 빌드 실패로 만들었습니다. \n모듈이 두꺼워졌을 때는 원인이 레이어 부재가 아니라 변경 이유가 다른 도메인이 섞인 것이라 생각해 도메인 축으로 다시 쪼갰습니다.',
      // result:
      ...
      // },
      {
        ...
        problem:
          '웹 인증을 레거시 호환(단일 쿠키)으로 갈지 모바일과 같은 access+refresh로 갈지가 프론트엔드 리드의 결정에 걸려 있는 상황에서 그쪽 일정 때문에 답을 바로 받기 어려웠습니다. 이 결정에 따라 인증 코어를 어떻게 구축할지가 바뀔 수 있어 당장 진행해야 하는 모바일 인증 작업까지 함께 멈추게 되는 상황이었습니다. 그렇다고 서버가 임의로 정해 두면 나중에 요구사항이 바뀔 때 인증 코어를 다시 작업해야 했습니다.',
        judgment:
          '기다리지 않고 모바일을 먼저 완성하되 인증 코어를 전송수단 중립으로 두어 웹이 나중에 붙을 수 있게 진행하기로 정했습니다. 그 선택으로 생기는 리스크 7개를 비용 기준으로 정리하고 Google audience 다중화와 JWT 클레임 중립, refresh 저장소 확장 필드, CSRF 삽입 지점, 웹 리다이렉트 골격처럼 작업이 가능한 다섯가지는 바로 구현했습니다. \n나머지는 코드로 해결되지 않는 약속과 운영의 영역이었습니다.',
        result:
          '가장 큰건 refresh 폐기가 모바일에만 적용되어 강제 로그아웃과 권한 회수가 웹에서는 최대 24시간 늦어지는 것 이었습니다. 그래서 "세션 폐기에 의존하는 기능"을 계약에 넣지 않기로 정해 약속의 범위를 좁혔습니다. \n이중 운영은 서버가 혼자 감당할 수 없어 사전 합의 대상으로 올리고 한 웹 세션이 두 서버를 섞어 부르지 않는다는 전제로 정리했습니다. 전환기 세션은 컷오버 전에는 신규 서버가 레거시 쿠키를 수용하고 이후에는 레거시가 신규 토큰을 수용하도록 협의를 했습니다. 덕분에 결정이 안 난 채로도 모바일을 먼저 완성할 수 있었습니다.',
      },
    ],
  },
];
