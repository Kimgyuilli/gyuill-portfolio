...
PeekCart에서는 대용량 이커머스 상황을 가정해 Redis 분산 락, DB 낙관적 락, Transactional Outbox, Kafka DLQ, GKE 부하 테스트까지 직접 설계하고 검증했습니다. 상품 조회 TPS 2.31배 개선(265 -> 613), CPU saturation 시 HPA 1->3 자동 확장처럼 수치로 확인 가능한 결과를 남겼고, 목표에 미달한 측정도 원인 분석과 함께 그대로 기록했습니다.
...
Result: k6로 1,000 VUser 동시 주문을 걸어 재고 정합성 `consistency=OK`, 오버셀링 0건을 확인했습니다. 다만 단일 노드(e2-standard-4) 환경에서 실패율 35.9%로 처리량 합격선(`http_req_failed < 0.1`)에는 미달했고, 커밋된 주문이 110건에 그쳐 재고 소진 경합까지는 도달하지 못한 점을 한계로 기록했습니다. 병목은 MySQL 커넥션 풀과 분산 락 contention으로 식별해 후속 과제로 남겼고, Redis 경합 실패와 Redis 장애 상황은 각각 다른 실패 모드로 분리해 ADR 및 블로그로 정리했습니다.
...
Result: 50 VUser 5분 기준으로 상품 조회 TPS를 265.0에서 612.7로 2.31배 개선했고, 평균 응답 시간을 188ms에서 82ms로 56.5% 단축했습니다. 목표였던 3배에는 미달했지만, 캐시 ON 상태에서도 CPU가 175%까지 올라 단일 Pod의 CPU 병목이 원인임을 확인하고 다음 HPA 검증 시나리오의 근거로 활용했습니다.
